...
prepare_plugins_and_workspace(){
    ...
    cp /vagrant/provisioning/groovy/create_all_jobs.groovy \
        /var/lib/jenkins/workspace/seed-job/jobs/create_all_jobs.groovy && \
...
get_crumb(){
    ...
    if [[ -f /var/lib/jenkins/wizard_completed ]]; then
        ...
        -s http://localhost:8080/crumbIssuer/api/json | jq -r .crumb
    else
        curl --user "admin:$1"\
        -s http://localhost:8080/crumbIssuer/api/json | jq -r .crumb
    fi
...
generate_seed_job_and_execute(){
    ...
    curl --user "admin:admin" \
        -d "$crumb_pass_reset" \
        --data-urlencode "script=$(< /vagrant/provisioning/groovy/jenkins_config_machine.groovy)" \
        http://localhost:8080/scriptText
    curl -XPOST \
        --user "admin:admin" \
        -d "$crumb_pass_reset" \
        http://localhost:8080/job/seed-job/build?token=TOKENVANDHRV73hbc5dsj
}
